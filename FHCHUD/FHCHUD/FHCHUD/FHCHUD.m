...
- (void)layoutSubviews{
    ...
    CGRect indicatorF = _indicatorView.bounds;
    ...
    _indicatorView.frame = indicatorF;
...
//    CGContextSetGrayFillColor(context, 0.0f, 0.8);
    CGContextSetFillColorWithColor(context, [UIColor colorWithWhite:0.793 alpha:1.000].CGColor);
    
    //center HUD
    CGRect allRect = self.bounds;
    //draw rounded HUD background rect
    CGRect boxRect = CGRectMake((round(allRect.size.width - _size.width) / 2), round((allRect.size.height - _size.height) / 2), _size.width, _size.height);
    ...
    CGContextAddArc(context, CGRectGetMinX(boxRect) + radius, CGRectGetMinY(boxRect) + radius, radius, (float)M_PI, 3 * (float)M_PI, 0);
